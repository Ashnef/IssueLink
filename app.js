  ...
  setTimeout(()=>{
    try{
      ...
      const report = {
        ...
        Severity: R.data.Severity, PhotoURLs: [R.photoDataUrl],
        SubmittedAt: Date.now(), Status:'Reported', AnonymousReporterID: ilId('ANON-',5)
      };
      reports.push(report);
      const ok = ilSave(IL_KEYS.reports, reports);
      if(!ok){ ilToast('Unable to save report — storage may be full.','error'); submitting=false; btn.disabled=false; btn.innerHTML='<i class="fa-solid fa-paper-plane"></i> Submit Report'; return; }
      document.getElementById('doneCode').textContent = report.ReportCode;
      showStep(6);
      ilToast('Report submitted successfully.','success');
    }catch(e){ console.error(e); ilToast('Unable to save report.','error'); }
