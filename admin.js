...
function doLogin(){
  ...
  const admin = admins.find(a=>a.Email.toLowerCase()===email && a.Password===pass && a.Status!=='Inactive');
...
function renderSettings(){
  ...
  document.getElementById('syncStatusText').textContent = settings.storageMode==='Google Sheets Sync' ? 'Enabled (manual sync)' : 'Local Only';
...
function saveDataSyncSettings(){
  ...
  settings.scriptUrl = document.getElementById('scriptUrlInput').value.trim();
...
async function syncNow(){
  ...
  if(settings.storageMode!=='Google Sheets Sync' || !settings.scriptUrl){
    ilToast('Enable Google Sheets Sync and set a script URL first.','error');
    return;
  }
  ...
  ilToast('Synchronizing with Google Sheets…','info');
  try{
    const payload = {
      action:'syncAll',
    ...
    // Merge the server result into local storage so records created elsewhere appear locally.
    ...
    if(Array.isArray(result.admins) && result.admins.length){
      const st=ilGetSettings(); st.admins=result.admins; ilSave(IL_KEYS.settings,st);
    }
    const updated = ilGetSettings();
    updated.lastSync=Date.now();
    ilSave(IL_KEYS.settings, updated);
    ilToast('Synchronization completed successfully.','success');
    goSection(A.section || 'dashboard');
  }catch(err){
    console.error('IssueLink sync error:',err);
    ilToast('Synchronization failed — Local Mode remains active. Check the Web App URL, deployment access, and Apps Script code.','error');
  }finally{
...
function addCategory(){
  ...
  const cats = ilLoad(IL_KEYS.categories, []);
  cats.push({CategoryID:ilId('C',5), CategoryName:name, Description:'', Active:true});
  ilSave(IL_KEYS.categories, cats);
  input.value=''; ilToast('Category added.','success'); renderCategoriesAdmin();
}
...
function addDepartment(){
  ...
  const deps = ilLoad(IL_KEYS.departments, []);
  deps.push({DepartmentID:ilId('D',5), DepartmentName:name, Description:'', Active:true});
  ilSave(IL_KEYS.departments, deps);
  input.value=''; ilToast('Department added.','success'); renderDepartmentsAdmin();
}
...
function toggleAdminStatus(id){
  ...
  if(idx!==-1 && s.admins[idx].Role!=='Super Admin'){ s.admins[idx].Status = s.admins[idx].Status==='Active'?'Inactive':'Active'; ilSave(IL_KEYS.settings,s); renderAdminsAdmin(); }
}
function addAdmin(){
  ...
  if(s.admins.some(a=>a.Email.toLowerCase()===email.toLowerCase())){ ilToast('An admin with that email already exists.','error'); return; }
  s.admins.push({AdminID:ilId('A',5), Name:name, Email:email, Password:pass, Role:role, Department:'', Status:'Active', CreatedAt:Date.now(), LastLogin:null});
  ilSave(IL_KEYS.settings, s);
  ...
  ilToast('Admin account created.','success');
  renderAdminsAdmin();
}
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if(aRequireAuth()) goSection('dashboard');
});
